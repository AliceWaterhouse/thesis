...
\section{Para--$c$--projective geometry}
...
\mynote{define complex and minimal and also para Hermitian. Should I retitle this section to reflect the fact that Hermitian metrics have nothing to do with c-projective geometry? Is that even true?}

%the equivalence class of connections
%defining the $c$--projective structure must be \textit{complex} and
%\textit{minimal}. Recall that a connection on an almost
%complex manifold $(M,J)$ is called \textit{complex} if it preserves $J$ and \textit{minimal} if the torsion is just the Nijenhuis tensor of $J$ up to a constant factor.
