...
\mynote{I am a bit confused because a contact form requires complete non--integrability of the hypersurface distribution, but \cite{CG} is saying that Nijenhuis taking asymptotically tangential values means the boundary structure is partially integrable. Oh but I guess integrability of the hypersurface distribution (dim 2n-2) is different from integrability of the $J$ eigen--bundles on $\p M$ (dim n-1)}
...
\mynote{I need to read a bit more of \cite{CGH-duke} to understand this section.}
...
\mynote{This section is nice in that it applies orbit decomposition \cite{CGH-duke} to the model. But only to show $g$ is Einstein, not that it's para--Kahler, and it doesn't mention the boundary geometry. So I'm not really sure if it's worth keeping.}
