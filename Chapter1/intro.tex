...
\widehat{W}_{ijk}^{\ \ \ l} = W_{ijk}^{\ \ \ l}, \qquad \widehat{\Rho}_{ij} = \Rho_{ij} - \nabla_i\Upsilon_j + \Upsilon_i\Upsilon_j
\ee
...
\mynote{Have we introduced $M$ yet?}
...
One consequence of theorem \ref{thm:DM} is that for every open set $\mathcal{U}\subset N$
...
which preserve $\theta$ and are equivariant under the principal $H$-action. \mynote{Maybe I should say this in the Cartan geom section? Thomas' Warsaw notes will probs say it.}
