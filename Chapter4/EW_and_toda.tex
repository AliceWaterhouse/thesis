...
\ee
where $d\mathcal{A}=\Omega$, and $\Omega$ is the symplectic form on $M$. Writing $\Sigma_{ab}=\iota_A\iota_B\epsilon_{A'B'}$, (\ref{beta_eq}) implies (\ref{dm3}) for a rescaling of $\mathcal{A}$, so it is the anti--self--duality of $\Omega$ which makes $D$ ASD in the sense of Calderbank. \mynote{I believe this is the first time we have mentioned the ASD'ty of $\Omega$. Can we also justify this? It is justified in section \ref{sec:model} for the model, but not in general.}
...
\]
\end{defi}
\mynote{h is also the name of a metric in c-proj...}
...
To understand $(M,[g])$ from the twistor perspective, we need to move to the complex picture. In what follows, we will view $M$ as the set of non--incident pairs in $\CP^2\times\CP^{2*}$. \mynote{Is it obvious that this is the right thing to do?} Let $F_{12}(\mathbb{C}^3)\subset \CP^2\times {\CP^2}^*$ be set of incident pairs 
...
is given by (\ref{tdistribution}). This implies that $L_{A}H=L_{A}\widetilde{H}$. Since each side of this equation is holomorphic on an open subset of $\CP^1$, and since $\CP^1$ can be covered with such subsets, both sides are globally holomorphic and therefore linear in $\pi^{A'}$ by the Liouville theorem. Hence
\[
