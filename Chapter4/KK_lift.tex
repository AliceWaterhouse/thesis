...
\label{chap:KK_lift}
In this chapter, we show that there is a canonical Einstein metric on an $\R^*$ bundle over $M$, with a connection whose curvature is the pull--back
...
We first note that the flat projective structure on $\mathbb{RP}^{n}$ gives a one--parameter family of $2n$--dimensional Einstein spaces $M$ which are so--called Kaluza--Klein reductions of quadrics in $\R^{2n+2}$ endowed with a flat neutral signature metric. When $\R^{2n+2}$ carries such a metric, we call it $\mathbb{R}^{n+1,n+1}$. Note that the phrase Kaluza--Klein refers to a classical unified field theory of gravitation and electromagnetism. We will use it to mean the construction of $g$ from a metric in dimension $2n+1$ of the form
\[
g \pm f^2(A+dt)^2,
\]
where $g$, $A$ and $f$ are a metric, Maxwell potential and function on $M$.

For $N=\RP^n$, the family $\{(g_\Lambda,\Omega_\Lambda)\,;\,\Lambda\neq 0\}$ is given by
\begin{eqnarray}
...
We now return to a general projective structure $(N,[\nabla])$, corresponding to the metric and symplectic form (\ref{eq:general_g},\ref{eq:general_omega}). Since
...
We reserve Latin indices $a,b,\dots$ for the $2n$-metric components
...
\[
-\Lambda(\delta^i_ke^{j+n}\wedge e^{k}+\delta_{j}^{i}e^{k+n}\wedge e^{k})=\frac{1}{2}R_{l\, m+n\, j}^{\qquad\ i}\,e^{l}\wedge e^{m+n}+\frac{1}{2}R_{m+n\, l\, j}^{\qquad\ i}\,e^{m+n}\wedge e^{l},
\]
so we conclude
\[
