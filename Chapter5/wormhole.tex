...
\]
where $x^a=(t,x)$ are coordinates on $\mathbb{R}^{1,1}$ and $\eta^{ab}$ is the Minkowski metric with signature $(-,+)$. The potential $U(\phi)=(1-\phi^2)^2/2$ is shown in figure \ref{fig:potential}. Note that it has two vacua, given by $\phi=\pm 1$. Finiteness of the associated conserved energy
\[
