...
trace-free Ricci curvature, and $-24\Lambda$\mynote{This minus sign...?} is the scalar curvature which acts
...
\ee
where $\{\tilde{\mathsf{e}}_{AA'}\}$ are the lifts of the null tetrad $\{\mathsf{e}_{AA'}\}$ to $\spp'$ (which are uniquely defined from the spin connection on $\spp'$ inherited from the Levi--Civita connection of $g$ on $TM$). We call $\mathcal{D}$ the \textit{twistor distribution}. \mynote{Write what the lifts of the null tetrad are.}
...
Any rank $k$ holomorphic vector bundle over $\CP^1$ is isomorphic to a direct sum of $k$ complex line bundles $\mathcal{O}(m_i),1\leq i\leq k$, each with first Chern class $m_i$. 
\end{theo}
...
A general ASD metric depends, in the real--analytic category, on six arbitrary functions of three variables. \mynote{Cite Ferepontov, Kruglikov, MD} Theorem \ref{thm:DM} gives an explicit subclass of such metrics which are additionally constrained by the following local characterisation.

\begin{theo}[\cite{DM}] \label{thm:DMcharacterisation}
Let $(M,g)$ be an real ASD Einstein manifold with scalar curvature $24$ admitting a totally null distribution $D_\beta$ which is ASD in the sense of Calderbank and parallel in the sense that $^{\bf g}\nabla_XY\in\Gamma(D_\beta)$ for all $X\in \Gamma(TM),\ Y\in\Gamma(D_\beta)$. Then $(M,g)$ is conformally flat, or it is locally isometric to (\ref{eq:g4}).
\end{theo}
\noindent
...
In three dimensions, the Einstein--Weyl equations give a set of five non--linear PDEs on the pair $(h, \omega)$ which are integrable by the twistor transform of Hitchin \cite{hitchin}.
...
\end{theo}
The conformal structure $[h]$ is obtained by demanding that a vector on $W$ is null if and only if the corresponding section of $\mathcal{O}(2)$ has a single zero. This condition is equivalent to the quadratic function $s(\lambda)$ which represents the section having vanishing discriminant.
...
lines) \mynote{by the canonical bundle we mean the bundle of volume forms.}, and restrict it to the intersection of (pre--images in
$T\CP^{2*}$
