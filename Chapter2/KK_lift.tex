...
p_{i}\mathrm{e}^{-t}, & \alpha=i=1,\dots,n\\
\mathrm{e}^{-t}\Bigl(\frac{1}{\Lambda}-x^{k}p_{k}\Bigr), & \alpha=n+1
...
\mathrm{e}^{-t}( dp_{i}-p_{i} dt), & \alpha=i=1,\dots,n\\
-\mathrm{e}^{-t}\biggl[\Bigl(\frac{1}{\Lambda}-x^{k}p_{k}\Bigr) dt+x^{k} dp_{k}+p_{k} dx^{k}\biggr], & \alpha=n+1.
\end{cases}
\end{eqnarray*}
The metric is then given by
\begin{eqnarray*}
G & = & \mathrm{e}^{t}( dx^{i}+x^{i} dt)\mathrm{e}^{-t}( dp_{i}-p_{i} dt)\ -\ \mathrm{e}^{t} dt\mathrm{e}^{-t}\biggl[\Bigl(\frac{1}{\Lambda}-x^{k}p_{k}\Bigr) dt+x^{k} dp_{k}+p_{k} dx^{k}\biggr]\\
 & = &  dx^{i} dp_{i}\ +\ (x^{i} dp_{i}-p_{i} dx^{i}) dt\ -\ (x^{i}p_{i}) dt^{2}\ -\  dt\biggl[\Bigl(\frac{1}{\Lambda}-x^{k}p_{k}\Bigr) dt+x^{k} dp_{k}+p_{k} dx^{k}\biggr]\\
 & = &  dx^{i} dp_{i}\ -\ \frac{1}{\Lambda} dt^{2}\ -\ 2p_{i} dx^{i} dt\\
 & = &  dx^{i} dp_{i}\ +\ \Lambda(p_{i} dx^{i})^{2}\ -\ \Lambda\Bigl(\frac{ dt}{\Lambda}+p_{i} dx^{i}\Bigr)^{2},
\end{eqnarray*}
